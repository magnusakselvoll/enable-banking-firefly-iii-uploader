...
public sealed class RepairState
{
    private readonly ConcurrentDictionary<string, (RepairPlan Plan, DateTimeOffset CreatedAt)> _pending = new();

    public string Add(RepairPlan plan)
    {
        PurgeExpired();
    ...
    public RepairPlan? TakeIfValid(string token)
    {
